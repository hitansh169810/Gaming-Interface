...
//	PreparedStatement pstmt2 = null ;
	...
	Font  highScoreFont  = new Font(Font.SANS_SERIF, Font.BOLD,  20);
	Font  highestScoreFont  = new Font(Font.SANS_SERIF, Font.BOLD,  20);
	...
	private void showScores(Graphics g) {
		showScore = userName+" : your Current Score is "+score ;
		g.setFont(scoreFont);
		g.setColor(Color.white);
		g.drawString(showScore,GWIDTH/2-50 ,30);
		String showHighScore = "Your Highest score is "+highScore ;
		g.setFont(highScoreFont);
		g.setColor(Color.white);
		g.drawString(showHighScore,GWIDTH/2-150,GHEIGHT-50);
		String showHighestScore = "All Player : Highest score is "+highestScore ;
		g.setFont(highestScoreFont);
		g.setColor(Color.white);
		g.drawString(showHighestScore,200,30);
	...
	private void doLast() {
		System.out.println(" score is "+score+" And highScore is "+highScore);
		sethighScore();
	}
	...
	private void sethighScore() {
		if(highScore <= score ) {
			...
			try {
				...
				con = DbHandler.getConnnection() ;

				if(con != null) {
					System.out.println("con is "+con);	
					getHighScore() ;

					pstmt = con.prepareStatement("update user_mst set snakegamescore ="+highScore+" where username=?") ;
	...
	private void getHighScore() {
		try {
			con = DbHandler.getConnnection() ;
			PreparedStatement pstmt = null ;
			pstmt = con.prepareStatement("select snakegamescore from user_mst where user_mst.username=?");
			pstmt.setString(1,userName);
			ResultSet rs = pstmt.executeQuery() ;
			while(rs.next()) {
				int dbHighScore = rs.getInt("snakegamescore") ;
				if(dbHighScore <= score) {
					dbHighScore = score ; 
				}
	...
	private void getHighestScore() {
		try {
			con = DbHandler.getConnnection() ;
			PreparedStatement pstmt = null ;
			pstmt = con.prepareStatement("select MAX(snakegamescore) from user_mst");
			ResultSet rs = pstmt.executeQuery() ;
			while(rs.next()) {
				int dbHighestScore = rs.getInt("max") ;
				if(dbHighestScore <= score) {
